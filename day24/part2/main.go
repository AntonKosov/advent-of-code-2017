...
func process(nodes []node) int {
	...
	longestBridge, bridgeStrength := 0, 0
	for _, startNodeIndex := range index[0] {
		...
		findMaxLength(nodes, index, cache, s)
		for s := range cache {
			length := aoc.CountBits(s.visited)
			if length > longestBridge || (length == longestBridge && s.strength > bridgeStrength) {
				longestBridge, bridgeStrength = length, s.strength
			}
		}
	}
...
func findMaxLength(nodes []node, index map[int][]int, cache map[state]struct{}, currentState state) {
	...
	for _, nextNodeIndex := range index[currentState.lastPort] {
		...
		findMaxLength(nodes, index, cache, nextState)
	}
}
